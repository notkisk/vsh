...
enum TokenKind { WORD, PIPE, REDIRECTION, AND_AND };
struct Token {
  std::string m_token;
...
std::vector<Path_Obj> retrievePath() {
  ...
  for (std::size_t i = 0; i <= path_view.size(); ++i) {
    if (i == path_view.size() || path_view[i] == ':') {
      ...
      paths.push_back(Path_Obj{path_part});
...
commandTokenizer(const std::string_view command) {
  ...
  while (i <= length) {
    ...
    if (command[i] == '\'' || command[i] == '\"') {
      is_quote_char = !is_quote_char;
      ++i;
      continue;
    }

    if (isWhiteSpace(command.substr(i, 1)) && !is_quote_char) {
      if (!current_token.empty()) {
        Token token;
        token.m_token = current_token;
        ...
        } else if (token.m_token == ">" || token.m_token == "<") {
...
std::optional<std::string> findExcutable(std::string_view command,
  ...
  for (const auto &path : paths) {
    ...
    std::string full_path = std::format("{}/{}", path.ex_path, command);
#ifdef DEBUG_MODE
...
void test_tokenizer(const std::string_view command,
  ...
  DEBUG_PRINT(std::format("number of tokens: {}. tokens {}", tokens.size(),
                          token_string));
}

std::vector<char *> tokens_to_argv(const std::vector<Token> &tokens) {
  // this one takes a series of tokens, and convert them to a c-string  vector
  // of arguments
...
int excuteCommand(std::string_view path, std::vector<char *> &argv) {
  pid_t pid = fork();
  if (pid == -1) {
    return -1;
  }
...
std::int16_t cd_builtin(const char *path) {
  ...
  if (path_view.starts_with("~/")) {
    ...
    std::string expanded{std::format("{}{}", home, path_view.substr(1))};
...
pwd_builtin(std::vector<char *> argv) noexcept {
  ...
  if (isLogical) {
    ...
    char *buffer = static_cast<char *>(std::malloc(logical_path.size() + 1));
    if (buffer == nullptr) {
      perror("pwd allocation failed");
      status = -1;
      return std::nullopt;
    }
    std::memcpy(buffer, logical_path.c_str(), logical_path.size() + 1);
  ...
  char *buffer = static_cast<char *>(std::malloc(physical_path.size() + 1));
  ...
  std::memcpy(buffer, physical_path.c_str(), physical_path.size() + 1);

  return std::make_tuple(buffer, status);
}

// TODO: add a sophisticated token parser/intreperter, reads tokens types and
// content (and probably index) and decides how to excute it!
...
#ifdef DEBUG_MODE
  test_tokenizer("echo arg1 arg2"sv, {"echo"sv, "arg1"sv, "arg2"sv});

  test_tokenizer("type  "sv, {"type"sv});

  test_tokenizer("exit       "sv, {"exit"sv});

  test_tokenizer("  echo arg1 arg2 arg3arg4  arg6"sv,
